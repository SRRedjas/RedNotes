...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:sidebar.header>
                <x-app-logo :sidebar="true" href="{{ auth()->check() ? route('dashboard') : route('wiki') }}" wire:navigate />
            ...
            @else
                <flux:button class="hidden lg:flex" :href="route('login')" icon="arrow-right-end-on-rectangle" variant="subtle" align="start" wire:navigate>
                    {{ __('Log in') }}
                </flux:button>
            @endauth
        </flux:sidebar>
        ...
        <flux:header class="lg:hidden">
            ...
            @else
                <flux:button :href="route('login')" icon="arrow-right-end-on-rectangle" size="sm" wire:navigate>
                    {{ __('Log in') }}
                </flux:button>
            @endauth
        </flux:header>
